...
def FedProto_taskheter(args, train_dataset, test_dataset, user_groups, user_groups_lt, local_model_list, classes_list):
    ...
    # record the global prototypes of every class
    global_protos = []
    # client index
    idxs_users = np.arange(args.num_users)


    # record the global losses and acc
    ...
    for round in tqdm(range(args.rounds)):
        ...
        proto_loss = 0
        # just the index of clients
        for idx in idxs_users:
            # idxs represent the indexs of the images per client allocated
            ...
            # the proto represent the local prototypes
            agg_protos = agg_func(protos)

            local_weights.append(copy.deepcopy(w))
            local_losses.append(copy.deepcopy(loss['total']))

            # record the local protos
        ...
        for idx in idxs_users:
            ...
            # store the parameters of model in this round
