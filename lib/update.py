...
import copy
import numpy as np
from lib.models.models import CNNFemnist
    ...
    def update_weights_het(self, args, idx, global_protos, model, global_round=round):
        ...
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, label_g) in enumerate(self.trainloader):
                ...
                # the first one is the softmax logits , the second one is the feature-map or so-called prototype of
                log_probs, protos = model(images)

                # the first loss function about cross-entropy
                loss1 = self.criterion(log_probs, labels)

                loss_mse = nn.MSELoss()

                # if there is no global prototypes, then skip
                if len(global_protos) == 0:
                    loss2 = 0*loss1

                else:
                    # copy the prototypes of one batch_size
                    ...
                    # here to calculate the loss between global prototypes and the local one
                    loss2 = loss_mse(proto_new, protos)

                # weight of the prototype loss
        ...
        # return the model parameters and so on. The last feedback is the local prototype
